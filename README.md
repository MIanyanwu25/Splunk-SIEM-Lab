...
| **5. Port 9997 NSG rule** | Add inbound rule to Splunk VM NSG: port `9997`, source = Windows VM private IP, priority `320`, name `Allow-SplunkForwarder-WindowsVM` |
| **6. VNet Peering (if VMs are in different VNets)** | Azure portal → Virtual Networks → your Splunk VNet → Peerings → Add. Name the links descriptively: `splunk-vnet-to-ad-vnet` and `ad-vnet-to-splunk-vnet`. Wait for both to show status **Connected** before continuing |
